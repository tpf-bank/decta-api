...
// DeliveryAddress delivery address fields
type DeliveryAddress struct {
	...
	// Full number of a cardholder for DHL delivery (including country code)
	Phone *string `json:"phone,omitempty"`
	// Communication language (ISO 639-1)
	Language *string `json:"language,omitempty"`
}

type _DeliveryAddress DeliveryAddress
...
func (o *DeliveryAddress) UnmarshalJSON(data []byte) (err error) {
	...
	varDeliveryAddress := _DeliveryAddress{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varDeliveryAddress)

	if err != nil {
		return err
	}

	*o = DeliveryAddress(varDeliveryAddress)
