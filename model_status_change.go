...
// checks if the StatusChange type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &StatusChange{}

// StatusChange Status change parameters
type StatusChange struct {
	// Desirable card state
	CardState string `json:"cardState"`
	// For <em>Block/Unblock</em> operation status change reason. For card <em>Activation</em> must contain last 6 digits of card pan.
	Details string `json:"details"`
}

type _StatusChange StatusChange
...
// GetCardState returns the CardState field value
...
// GetCardStateOk returns a tuple with the CardState field value
...
func (o StatusChange) ToMap() (map[string]interface{}, error) {
...
func (o *StatusChange) UnmarshalJSON(data []byte) (err error) {
	...
	varStatusChange := _StatusChange{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varStatusChange)

	if err != nil {
		return err
	}

	*o = StatusChange(varStatusChange)
