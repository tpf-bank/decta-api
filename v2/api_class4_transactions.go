...
// Class4TransactionsAPIService Class4TransactionsAPI service
type Class4TransactionsAPIService service

type ApiCardTransactionsRequest struct {
	ctx context.Context
	ApiService *Class4TransactionsAPIService
	requestId *string
	tokenHeader *string
	tokenSignature *string
	ppan string
	includeAccountTransactions *bool
	holdNumber *string
	trnTime *[]DateTimeFilter
	count *int32
	startId *int64
}

// Request ID (UUID format)
func (r ApiCardTransactionsRequest) RequestId(requestId string) ApiCardTransactionsRequest {
	r.requestId = &requestId
	return r
}

// URL64Encoded without padding Header part of JWS token
func (r ApiCardTransactionsRequest) TokenHeader(tokenHeader string) ApiCardTransactionsRequest {
	r.tokenHeader = &tokenHeader
	return r
}

// URL64Encoded without padding Signature part of JWS
func (r ApiCardTransactionsRequest) TokenSignature(tokenSignature string) ApiCardTransactionsRequest {
	r.tokenSignature = &tokenSignature
	return r
}

// Include account transactions into result transactions list
func (r ApiCardTransactionsRequest) IncludeAccountTransactions(includeAccountTransactions bool) ApiCardTransactionsRequest {
	r.includeAccountTransactions = &includeAccountTransactions
	return r
}

// Allows to select transactions from a  list, by providing an id of matched authorization lock which was released by receiving transaction information from the payment system. Allowed filter operator(s): eq, ne
func (r ApiCardTransactionsRequest) HoldNumber(holdNumber string) ApiCardTransactionsRequest {
	r.holdNumber = &holdNumber
	return r
}

// The filter allows users to specify transaction time. Users can select multiple time frames. Allowed filter operator(s): eq, ne, gt, ge, lt, le
func (r ApiCardTransactionsRequest) TrnTime(trnTime []DateTimeFilter) ApiCardTransactionsRequest {
	r.trnTime = &trnTime
	return r
}

// Number of transactions
func (r ApiCardTransactionsRequest) Count(count int32) ApiCardTransactionsRequest {
	r.count = &count
	return r
}

// Start the ordered list from transaction with selected transaction id
func (r ApiCardTransactionsRequest) StartId(startId int64) ApiCardTransactionsRequest {
	r.startId = &startId
	return r
}

func (r ApiCardTransactionsRequest) Execute() (*TransactionInfoDataArray, *http.Response, error) {
	return r.ApiService.CardTransactionsExecute(r)
}

/*
CardTransactions Get card transactions

The request allows users to search and receive card transactions for specific PPAN.

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param ppan Masked card number
 @return ApiCardTransactionsRequest
*/
func (a *Class4TransactionsAPIService) CardTransactions(ctx context.Context, ppan string) ApiCardTransactionsRequest {
	return ApiCardTransactionsRequest{
		ApiService: a,
		ctx: ctx,
		ppan: ppan,
...
//  @return TransactionInfoDataArray
func (a *Class4TransactionsAPIService) CardTransactionsExecute(r ApiCardTransactionsRequest) (*TransactionInfoDataArray, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *TransactionInfoDataArray
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "Class4TransactionsAPIService.CardTransactions")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/v1/api/cards/{ppan}/transactions"
	...
	if r.holdNumber != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "holdNumber", r.holdNumber, "form", "")
	}
	if r.trnTime != nil {
		t := *r.trnTime
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				parameterAddToHeaderOrQuery(localVarQueryParams, "trnTime", s.Index(i).Interface(), "form", "multi")
			}
		} else {
			parameterAddToHeaderOrQuery(localVarQueryParams, "trnTime", t, "form", "multi")
		}
	}
	if r.count != nil {
...
type ApiDoTransactionRequest struct {
	ctx context.Context
	ApiService *Class4TransactionsAPIService
	requestId *string
	tokenHeader *string
...
// Request ID (UUID format)
func (r ApiDoTransactionRequest) RequestId(requestId string) ApiDoTransactionRequest {
	r.requestId = &requestId
	return r
}

// URL64Encoded without padding Header part of JWS token
func (r ApiDoTransactionRequest) TokenHeader(tokenHeader string) ApiDoTransactionRequest {
	r.tokenHeader = &tokenHeader
	return r
}

// URL64Encoded without padding Signature part of JWS
func (r ApiDoTransactionRequest) TokenSignature(tokenSignature string) ApiDoTransactionRequest {
	r.tokenSignature = &tokenSignature
	return r
}

func (r ApiDoTransactionRequest) Transaction(transaction Transaction) ApiDoTransactionRequest {
	r.transaction = &transaction
	return r
}

func (r ApiDoTransactionRequest) Execute() (*TransactionInfo, *http.Response, error) {
	return r.ApiService.DoTransactionExecute(r)
}

/*
DoTransaction Process card transaction

The request allows users to process card top-up or withdrawal transactions.

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param ppan Masked card number
 @return ApiDoTransactionRequest
*/
func (a *Class4TransactionsAPIService) DoTransaction(ctx context.Context, ppan string) ApiDoTransactionRequest {
	return ApiDoTransactionRequest{
		ApiService: a,
		ctx: ctx,
		ppan: ppan,
...
func (a *Class4TransactionsAPIService) DoTransactionExecute(r ApiDoTransactionRequest) (*TransactionInfo, *http.Response, error) {
	var (
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "Class4TransactionsAPIService.DoTransaction")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}
